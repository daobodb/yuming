// ==========================================
// yutian81/domain-check 官方高保真全功能像素级复刻代码（大满贯完全体版）
// ==========================================
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/cron') {
      ...
      return new Response('Manual Cron execution finished.');
    }
    ...
    const config = {
      ...
      GITHUB_URL: env.GITHUB_URL || DEFAULT_CONFIG.GITHUB_URL,
...
function renderOriginalStyleHTML(domains, config, rawDomains) {
  ...
  const backupJsonStr = config.isAuthed ? encodeURIComponent(JSON.stringify(rawDomains)) : '';
  ...
  const cardsHtml = domains.map((d, index) => {
    ...
    // 分组默认处理：如果没有填写，或者选了未分组，统一规范为“未分组”
  ...
  <body>
    <div class="panel">
      ...
      <div class="matrix-grid" id="cardMatrix">
        ${cardsHtml || '<div style="grid-column: 1/-1; text-align:center; padding:40px; color:#64748b;">没有找到匹配的监控节点。</div>'}
      </div>
    </div>
    ...
    <script>
      ...
      function exportData() {
        const raw = "${backupJsonStr}";
        if(!raw) return;
        const blob = new Blob([decodeURIComponent(raw)], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'domains_backup_' + new Date().toISOString().split('T')[0] + '.json';
        a.click();
      }
